...
class CNN(torch.nn.Module):
    def __init__(self, num_bins): 
        ...
        #3 input channels (3 color channels), 48 output channels
        self.conv1 = torch.nn.Conv2d(3, 48, stride=3, kernel_size=(7,7)) 

        #Kernel size of 3,3 and stride is 3 (shift 3 pixels to the right)
        self.pool1 = torch.nn.MaxPool2d((3,3),stride=1)

        #48 input channels, 24 output channels, adjusted kernel
        self.conv2 = torch.nn.Conv2d(48, 24, stride=3, kernel_size=(5,5)) 

        #Add batch normalization after ReLu with same number of features as conv2
        self.batch1 = torch.nn.BatchNorm2d(24) 

        #24 input channels, 12 output channels, adjusted kernel
        self.conv3 = torch.nn.Conv2d(24, 12, stride = 1, kernel_size=(2,7))

        #Add batch normalization after ReLu with same number of features as conv3
        self.batch2 = torch.nn.BatchNorm2d(12) 

        #Final Conv to set output channels = num_bins and modifying kernel to be equal to the previous layer output 
        self.conv4 = torch.nn.Conv2d(12,num_bins, kernel_size=(4,17))
    ...
    def forward(self, x):
        ...
        #Order:
        #CONV->POOL->CONV->ReLu->BATCH->DROP->CONV->ReLu->BATCH->DROP->CONV
...
class dataloader(torch.utils.data.Dataset):
    ...
    def normalize_to_zero_mean(self):
        #---FILL ME IN-----
        #Subtracting the mean of the training set from both the input and validation images?
        #This doesn't make any sense doing it here please lmk how u do this after the assignment plz at: avram.kachura@mail.utoronto.ca
        #------------------
        pass
...
if __name__ == "__main__": 
    ...
    #Should get good results within 30 epochs but iterated through more get more training with weights
